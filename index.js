...
function processDuplicateFree(list, cb) {
  let newList = list.filter((v,i) => list.indexOf(v) === i);
  return cb(newList);
}
...
function isItAnApple(strings) {
  const mappedStrings = strings.map(i => i.includes("apple"));
...
function removeApple(strings) {
  let newStrings = strings.filter(i => i != "apple");
...
function stringSmash(strings) {
  let newString = strings.reduce(function(p,c,i) {
    return i == 0 ? c : p + c;
  });
  return newString;
}
...
function getFullNames(runners) {
  const newRunners = [];
  runners.forEach(i => newRunners.push(i.last_name + ", " + i.first_name));
  return newRunners;
}
...
function firstNamesAllCaps(runners) {
  const newRunners = runners.map(i => i.first_name.toUpperCase());
  return newRunners;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  let newRunners = runners.filter(i => i.shirt_size === tShirtSize);
  return newRunners;
}
